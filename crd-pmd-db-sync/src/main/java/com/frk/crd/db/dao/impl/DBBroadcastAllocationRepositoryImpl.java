...
//      + " and crd_owner.FRK_GETORDERFLOWTYPE(o.order_id) = 'SMEF' "
  ...
  @Override
  public List<DBBroadcastAllocation> getAllocations(String orderId) {
    final List<DBBroadcastAllocation> allocations = new ArrayList<>();
    jdbcTemplate.query(this.ALLOCATION_SQL, preparedStatementSetter -> {
          preparedStatementSetter.setString(1, orderId);
        },
        resultSet -> {
          DBBroadcastAllocation allocation = new DBBroadcastAllocation();
          allocation.setOrigOrderId(resultSet.getString(1));
